//! Version 5 .rm parser - Layer-based format with transforms
//!
//! This format was used in firmware 2.6 through 2.x.
//! Adds layer support with per-layer transforms (translate/scale/rotate).
//!
//! File structure:
...
use crate::versions::{RmParser, HEADER_V5, HEADER_SIZE};

/// Parser for .rm format version 5
pub struct V5Parser {
    data: Vec<u8>,
}
        ...
        for _ in 0..num_layers {
            ...
            for _ in 0..num_strokes {
                ...
                let base_width = cursor.read_f32::<LittleEndian>()
                    .map_err(|_| LinesError::UnexpectedEof)?;
                // v5 adds an extra unknown field
                let _unknown2 = cursor.read_i32::<LittleEndian>()
                    .map_err(|_| LinesError::UnexpectedEof)?;
